# projet Flask Gestion Produits

Ce projet est une application web simple de gestion de produits  je fais des operations CRUD(Creer Lister Modifier Suppprimer) a l'interieur et j'ai utilisé Sqlite pour la base de donnée et je fais aussi une connexion sans autotisation 
# Fonctionnalités principales

-  Ajout de nouveaux produits
-  Modification et mise à jour des produits
-  Suppression de produits
-  Liste des produits affichée dynamiquement
-  Filtrage dynamique des produits via JavaScript
-  Connexion utilisateur avec Flask-Login (sans inscription)


#  Compte utilisateur par défaut

Pour se connecter, utilisez les identifiants suivants (utilisateur pré-initialisé) :

Email :`booba@example.com`  
Mot de passe :`password1`

# Lancer le projet

pour lancer le projet faites flask run --debug

# Technologies utilisées

- Flask / Flask-Login / Flask-WTF / Flask-Migrate / SQLAlchemy
- SQLite (base de données)
- Tailwind
- JavaScript (pour le filtrage dynamique)







